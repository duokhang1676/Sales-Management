...
public class TaoPhieuXuatTraPage extends javax.swing.JPanel {
	...
	private PhieuXuatTraDao phieuXuatTra_dao = new PhieuXuatTraDao();
	private HangHoaDao hangHoa_dao = new HangHoaDao();

	private ChiTietPhieuXuatTraDao chiTietPNH_dao = new ChiTietPhieuXuatTraDao();
	...
	public void setTable() {
    	TableColumn column = tbl_HH.getColumnModel().getColumn(8);
    ...
    private void addXuatTraTable() {
		// TODO Auto-generated method stub
    	
    	String[] colNames = {"STT", "Mã HH", "Tên HH", "Số lô","Ngày sản xuất", "Hạn sử dụng", "DVT","Số lượng lô" ,"Số lượng xuất", "Giá xuất", "Chiết khấu", "Thành tiền", "Xóa"};
        
        model_HH = new DefaultTableModel(colNames, 0);
        tbl_HH = new JTable(model_HH);
        FormatJtable.setFontJtable(tbl_HH);
        FormatJtable.setCellEditableForPXT(tbl_HH);
        ...
        tbl_HH.getColumnModel().getColumn(9).setCellEditor(new NumericCellEditor());
...
//							model_HH.setValueAt(loHH.getSoLuong(), row, 7);
                    ...
                    if (col == 8 || col == 9 || col == 10) { // Kiểm tra nếu cột "Số lượng" hoặc "Giá nhập" đã được thay đổi             
                        int soLuong = Integer.parseInt(model_HH.getValueAt(row, 8).toString());
                        double giaNhap = 0;
                        if(!model_HH.getValueAt(row, 9).toString().isEmpty())
                        giaNhap = Double.parseDouble(model_HH.getValueAt(row, 9).toString().replaceAll(",", ""));
                    ...
                    for (int i = 0; i < model_HH.getRowCount(); i++) {
                        if(!model_HH.getValueAt(row, 9).toString().isEmpty())
                        tienNhap = Double.parseDouble(model_HH.getValueAt(i, 9).toString().replaceAll(",", ""));
                        soLuong = Double.parseDouble(model_HH.getValueAt(i, 8).toString());
    ...
    private void initComponents() {
        ...
        txt_timLoHang = new javax.swing.JTextField();
        ...
        jL_timHangHoa.setText("Nhập số lô của lô hàng cần xuất:");
        ...
        txt_timLoHang.setFont(new java.awt.Font("Times New Roman", 0, 14)); // NOI18N
        txt_timLoHang.setPreferredSize(new java.awt.Dimension(400, 35));
        txt_timLoHang.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                txt_timLoHangActionPerformed(evt);
            }
        });
        jP_bodyHeader.add(txt_timLoHang, new org.netbeans.lib.awtextra.AbsoluteConstraints(30, 40, 520, -1));
    ...
    private void txt_timLoHangActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_txt_timLoHangActionPerformed
        // TODO add your handling code here:
    	int rowCount = model_HH.getRowCount();
    	if (txt_timLoHang.getText().trim().isEmpty()) 
    		return;
    	LoHang lh = loHang_dao.timLoHangTheoMa(txt_timLoHang.getText().trim());
    	
    	if(lh==null) {
    		showMessage("Số lô không tồn tại!");
			txt_timLoHang.selectAll();
			txt_timLoHang.requestFocus();
			return;
    	}else {
    		for(int i = 0;i<model_HH.getRowCount();i++) {
        		if(model_HH.getValueAt(i, 3).toString().equals(lh.getSoLo())) {
        			showMessage("Số lô đã có trong danh sách phiếu ghi!");
        			txt_timLoHang.selectAll();
        			txt_timLoHang.requestFocus();
        			return;
        		}
    		...
    		model_HH.addRow(new Object[] {rowCount+1,lh.getHangHoa().getMaHangHoa(), lh.getHangHoa().getTenHangHoa(),lh.getSoLo(),Formater.dateFormater(lh.getNgaySanXuat()),Formater.dateFormater(lh.getHanSuDung()),lh.getHangHoa().getDonViTinh(),lh.getSoLuong(), 1,0, 0, 0, ""});
			txt_timLoHang.setText("");
			txt_timLoHang.requestFocus();
    	}
    }//GEN-LAST:event_txt_timLoHangActionPerformed
    ...
    private void btn_taoPhieuNhap1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_taoPhieuNhap1ActionPerformed
    	...
    	if(checkTable()) {
        	...
        	String maPX = "PX"+System.currentTimeMillis();
        	PhieuXuatTra pxt = new PhieuXuatTra(maPX, tgTao, ghiChu, tongGiamGia, tongTienHang);
 	
        	
        	if (phieuXuatTra_dao.createPXT(pxt)) {
        		//tao chi tiet pnh
        		for (int i = 0; i < tbl_HH.getRowCount(); i++) {
        			...
        			int soLuong = Integer.parseInt(model_HH.getValueAt(i, 8).toString());
        			double giaXuat = Double.parseDouble(model_HH.getValueAt(i, 9).toString().replaceAll(",", ""));
        			...
        			loHang.setSoLuong(loHang.getSoLuong()-soLuong);
        			
        			
        			ChiTietPhieuXuatTra ctPXT = new ChiTietPhieuXuatTra(pxt, loHang, hh, soLuong, giaXuat,0);
        			
        			chiTietPNH_dao.addChiTietPXT(ctPXT);
        			
        			//Cập nhật số lượng
        			hangHoa_dao.capNhatSoLuongHangHoa(hh);
        			loHang_dao.capNhatSoLuongLoTheoMa(loHang);
    			...
    			showMessage("Tạo không thành công!");
    		}
    	}
    ...
    private boolean checkTable() {
    	...
    	for(int i = 0;i<model_HH.getRowCount();i++) {
    		int soLuongLo = Integer.parseInt(model_HH.getValueAt(i, 7).toString());
    		int soLuongXuat = Integer.parseInt(model_HH.getValueAt(i, 8).toString());

    		if(soLuongXuat>soLuongLo) {
    			showMessage("Số lượng xuất không được lớn hơn số lượng còn lại trong lô! Lỗi tại STT"+(i+1));
    			return false;
    		}
    	}
    	return true;
    ...
    private javax.swing.JTextField txt_timLoHang;
